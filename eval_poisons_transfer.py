...
def train_network_with_poison(net, target_img, target_camera_tuple, poison_tuple_list, poisoned_dset,
    ...
    for epoch in range(args.retrain_epochs):
        ...
        if epoch == args.retrain_epochs - 1:
            ...
            print("Now evaluating against camera taken images")
            res_camera = {}
            for target_camera_img, print_size in target_camera_tuple:
                if args.device == 'cuda':
                    target_camera_pred = net(target_camera_img.to('cuda'))
                else:
                    target_camera_pred = net(target_camera_img)
                target_camera_scores = [float(n) for n in
                                        list(softmax(target_camera_pred.view(-1).cpu().detach().numpy()))]
                target_camera_score, target_camera_pred = target_camera_pred.topk(1, 1, True, True)
                print(
                    "Target Label: {}, Poison label: {}, Prediction:{}, Target's Score:{}, Poisons' Predictions:{}".format(
                        args.target_label, args.poison_label, target_camera_pred[0][0].item(), target_camera_scores,
                        poison_pred_list))
                res_camera[print_size] = {"prediction": target_camera_pred[0][0].item(), 'scores': target_camera_scores,
                                          "malicious score": target_camera_scores[args.poison_label]}
    ...
    return {'clean acc': val_acc_meter.avg, 'prediction': target_pred[0][0].item(),
            ...
            'scores': target_scores, 'malicious score': target_scores[args.poison_label], 'camera': res_camera}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--target-camera', default=True, type=bool)
    parser.add_argument('--target-print-size', default=[2, 4], type=int, nargs="+",
                        help='width of the target when printed on the paper (in inch)')
    ...
    for target_idx in range(args.target_index_start, args.target_index_end, args.target_index_step):
        ...
        if args.target_camera:
            for print_size in args.target_print_size:
                target = fetch_camera_target(target_idx, print_size, transforms=transform_test)
        ...
        models_dir = '{}/{}/models-retrained-for-{}epochs/'.format(
        ...
        res = {}
        if args.end2end:
            ites = [1, 51, 101, 201, 301, 501, 701, 1101, 1500]
        else:
